...
async def update_progress(client,message,torrent,except_retry=0,sleepsec=None):
    ...
    while True:
        ...
        try:
            ...
            msg += f'<b>Down:</b> {human_readable_bytes(tor_info.dlspeed, postfix="/s")} <b>Up:</b> {human_readable_bytes(tor_info.upspeed, postfix="/s")}\n'
    ...
    if len(tors) > 0:
        ...
        for i in tors:
            if i.progress != 1 or all:
                olen += 1
                msg += f'📥 <b>{i.name} | {round(i.progress*100, 2)}% | {human_readable_bytes(i.completed)}/{human_readable_bytes(i.size)}({human_readable_bytes(i.total_size)}) | {human_readable_bytes(i.dlspeed, postfix="/s")} | {human_readable_timedelta(i.eta)} | S:{i.num_seeds} | L:{i.num_leechs} | {i.state}</b>\n\n'
